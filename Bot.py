    ...
    def run(self):
        while True:
            ...
            if isinstance(self.deck_type, DeckType) and self.deck_strategy is None:
                ...
                if (
                    self.deck_strategy.deck is None
                    and self.deck_type == DeckType.Pirates
                ):
...
#            sleep(1.5)
  ...
  def continue_and_replay(self):
    ...
    def play(self):
        ...
        if self.game_state == GameState.Mulligan:
            ...
            playable_cards = sorted(
                filter(
                    lambda card: card.cost <= self.mana
                    or card.is_spell()
                    and card.cost <= self.mana + self.spell_mana,
            ...
            if (
                ...
                if playable_card_in_hand:
                    ...
                    if "ally in hand" in playable_card_in_hand.description_raw:
                        ...
                        units_in_hand = tuple(
                            filter(
                                lambda card_in_hand: not card_in_hand.is_spell(),
    ...
    def select_deck(self):
        ...
        for val in vals:
            v = (
                self.window_x + val[0] * self.window_width,
                self.window_y + val[1] * self.window_height,
            )
        ...
        ok_button_pos = (
            self.window_x + 0.5 * self.window_width,
            self.window_y + 0.546 * self.window_height,
        )
